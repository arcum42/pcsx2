...
void recVUMI_RSQRT(VURegs *VU, int info)
{
    ...
    if (t1boolean)
        xMOVAPS(xRegisterSSE(t1reg), ptr[&RSQRT_TEMP_XMM[0]]); // restore t1reg data
    else
        XMM_Reg.freeReg(t1reg); // free t1reg
}
//------------------------------------------------------------------
...
void _addISIMMtoIT(VURegs *VU, s16 imm, int info)
{
    ...
    else
    {
        if (imm)
        {
            ...
            xMOV(xRegister32(itreg), xRegister32(isreg));
    }
}
//------------------------------------------------------------------
...
void recVUMI_IADD(VURegs *VU, int info)
{
    ...
    if (_Is_ == 0)
    {
        if ((itreg = X86_Reg.checkReg((x86type)(X86TYPE_VI | ((VU == &VU1) ? X86TYPE_VU1 : 0)), _It_, MODE_READ)) >= 0)
        {
            if (idreg != itreg) xMOV(xRegister32(idreg), xRegister32(itreg));
        }
        else
            xMOVZX(xRegister32(idreg), ptr16[(u16 *)(VU_VI_ADDR(_It_, 1))]);
    }
    else if (_It_ == 0)
    {
        if ((isreg = X86_Reg.checkReg((x86type)(X86TYPE_VI | ((VU == &VU1) ? X86TYPE_VU1 : 0)), _Is_, MODE_READ)) >= 0)
        {
            if (idreg != isreg) xMOV(xRegister32(idreg), xRegister32(isreg));
        }
        else
            xMOVZX(xRegister32(idreg), ptr16[(u16 *)(VU_VI_ADDR(_Is_, 1))]);
    }
    else
    {
        //ADD_VI_NEEDED(_It_);
        ...
        if (idreg == isreg)
            xADD(xRegister32(idreg), xRegister32(itreg));
        else if (idreg == itreg)
            xADD(xRegister32(idreg), xRegister32(isreg));
        else
            xLEA(xRegister32(idreg), ptr[xAddressReg(isreg) + xAddressReg(itreg)]);
...
void recVUMI_IAND(VURegs *VU, int info)
{
    ...
    if (idreg == isreg)
        xAND(xRegister16(idreg), xRegister16(itreg));
    else if (idreg == itreg)
        xAND(xRegister16(idreg), xRegister16(isreg));
    else
    {
...
void recVUMI_IOR(VURegs *VU, int info)
{
    ...
    else
    {
        ...
        if (idreg == isreg)
            xOR(xRegister16(idreg), xRegister16(itreg));
        else if (idreg == itreg)
            xOR(xRegister16(idreg), xRegister16(isreg));
        else
        {
...
void recVUMI_MOVE(VURegs *VU, int info)
{
    ...
    if (_X_Y_Z_W == 0x8)
        xMOVSS(xRegisterSSE(EEREC_T), xRegisterSSE(EEREC_S));
...
void recVUMI_MFIR(VURegs *VU, int info)
{
    ...
    X86_Reg.deleteReg((x86type)(X86TYPE_VI | ((VU == &VU1) ? X86TYPE_VU1 : 0)), _Is_, 1);
...
void recVUMI_MTIR(VURegs *VU, int info)
{
    ...
    X86_Reg.deleteReg((x86type)(X86TYPE_VI | ((VU == &VU1) ? X86TYPE_VU1 : 0)), _It_, 2);
...
void _loadEAX(VURegs *VU, int x86reg, uptr offset, int info)
{
    ...
    if (x86reg >= 0)
    {
        switch (_X_Y_Z_W)
        {
            ...
            case 15:
                if (VU == &VU1)
                    xMOVAPS(xRegisterSSE(EEREC_T), ptr[xAddressReg(x86reg) + offset]);
                else
                    xMOVUPS(xRegisterSSE(EEREC_T), ptr[xAddressReg(x86reg) + offset]);
                break;
            default:
                if (VU == &VU1)
                    xMOVAPS(xRegisterSSE(EEREC_TEMP), ptr[xAddressReg(x86reg) + offset]);
                else
                    xMOVUPS(xRegisterSSE(EEREC_TEMP), ptr[xAddressReg(x86reg) + offset]);
    ...
    else
    {
        switch (_X_Y_Z_W)
        {
            ...
            case 15:
                if (VU == &VU1)
                    xMOVAPS(xRegisterSSE(EEREC_T), ptr[(void *)(offset)]);
                else
                    xMOVUPS(xRegisterSSE(EEREC_T), ptr[(void *)(offset)]);
                break;
            default:
                if (VU == &VU1)
                    xMOVAPS(xRegisterSSE(EEREC_TEMP), ptr[(void *)(offset)]);
                else
                    xMOVUPS(xRegisterSSE(EEREC_TEMP), ptr[(void *)(offset)]);
                VU_MERGE_REGS(EEREC_T, EEREC_TEMP);
                break;
        }
    }
...
int recVUTransformAddr(int x86reg, VURegs *VU, int vireg, int imm)
{
    ...
    {
        if (imm)
            xLEA(eax, ptr[xAddressReg(x86reg) + imm]);
        else
...
void recVUMI_LQI(VURegs *VU, int info)
{
    ...
    if (_Ft_ == 0)
    {
        if (_Is_ != 0)
        {
            if ((isreg = X86_Reg.checkReg((x86type)(X86TYPE_VI | (VU == &VU1 ? X86TYPE_VU1 : 0)), _Is_, MODE_WRITE | MODE_READ)) >= 0)
            {
...
void _saveEAX(VURegs *VU, int x86reg, uptr offset, int info)
{
    ...
    if (_Fs_ == 0)
    {
        if (_XYZW_SS)
        {
            ...
            if (x86reg >= 0)
                xMOV(ptr32[xAddressReg(x86reg) + offset + (_W ? 12 : (_Z ? 8 : (_Y ? 4 : 0)))], c);
            else
                xMOV(ptr32[(u32 *)(offset + (_W ? 12 : (_Z ? 8 : (_Y ? 4 : 0))))], c);
        }
        else
        {
    ...
    switch (_X_Y_Z_W)
    {
        ...
        case 4: // Y
            ...
            if (x86reg >= 0)
                xMOVSS(ptr[xAddressReg(x86reg) + offset + 4], xRegisterSSE(EEREC_TEMP));
            else
                xMOVSS(ptr[(void *)(offset + 4)], xRegisterSSE(EEREC_TEMP));
            break;
        case 5: // YW
        ...
        case 6: // YZ
            ...
            if (x86reg >= 0)
                xMOVL.PS(ptr[xAddressReg(x86reg) + offset + 4], xRegisterSSE(EEREC_TEMP));
            else
                xMOVL.PS(ptr[(void *)(offset + 4)], xRegisterSSE(EEREC_TEMP));
            break;
        case 7:                                                              // YZW
        ...
        case 15: // XYZW
            ...
            {
                if (x86reg >= 0)
                    xMOVAPS(ptr[xAddressReg(x86reg) + offset + 0], xRegisterSSE(EEREC_S));
                else
                    xMOVAPS(ptr[(void *)(offset)], xRegisterSSE(EEREC_S));
            }
            else
            {
                if (x86reg >= 0)
                    xMOVUPS(ptr[xAddressReg(x86reg) + offset + 0], xRegisterSSE(EEREC_S));
                else
                {
                    if (offset & 15)
                        xMOVUPS(ptr[(void *)(offset)], xRegisterSSE(EEREC_S));
                    else
                        xMOVAPS(ptr[(void *)(offset)], xRegisterSSE(EEREC_S));
                }
            }
...
void recVUMI_ILW(VURegs *VU, int info)
{
    ...
    if (_X)
        ...
        pxAssertMsg(0, "off is 0 as default value, could be incorrect");
...
void recVUMI_RINIT(VURegs *VU, int info)
{
    ...
    {
        X86_Reg.deleteReg((x86type)(X86TYPE_VI | (VU == &VU1 ? X86TYPE_VU1 : 0)), REG_R, 2);
        _unpackVFSS_xyzw(EEREC_TEMP, EEREC_S, _Fsf_);
    ...
    else
    {
        ...
        X86_Reg.deleteReg((x86type)(X86TYPE_VI | (VU == &VU1 ? X86TYPE_VU1 : 0)), REG_R, 1);
    }
}
//------------------------------------------------------------------
...
void recVUMI_FMAND(VURegs *VU, int info)
{
    ...
    if (isreg >= 0)
    {
        ...
        xMOVZX(xRegister32(itreg), ptr16[(u16 *)(VU_VI_ADDR(_Is_, 1))]);
...
void recVUMI_ESADD(VURegs *VU, int info)
{
    ...
    { // special code to reset P ( FixMe: don't know if this is still needed! (cottonvibes) )
