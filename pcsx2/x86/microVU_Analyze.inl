...
__ri void analyzeReg2(mV, int xReg, microVFreg &vfWrite, bool isLowOp)
{
    ...
    {
        #define bReg(x, y)            \
            mVUregsTemp.VFreg[y] = x; \
            mVUregsTemp.VF[y]

        if (_X)
        {
            bReg(xReg, isLowOp).x = 4;
            vfWrite.reg = xReg;
            vfWrite.x = 4;
        }
...
__fi void mVUanalyzeLQ(mV, int Ft, int Is, bool writeIs)
{
    ...
    if (!Ft)
    {
        if (writeIs && Is) { mVUlow.noWriteVF = 1; }
        else
        {
            mVUlow.isNOP = 1;
        }
...
static void analyzeBranchVI(mV, int xReg, bool &infoVar)
{
    if (!xReg) return;
    if (mVUstall)
    { // I assume a stall on branch means the vi reg is not modified directly b4 the branch...
