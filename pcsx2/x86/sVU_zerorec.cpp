...
struct WRITEBACK
{
    WRITEBACK()
        : nParentPc(0)
        , cycle(0) //, pStatusWrite(NULL), pMACWrite(NULL)
    {
...
static VuBaseBlock *SuperVUBuildBlocks(VuBaseBlock *parent, u32 startpc, const VUPIPELINES &pipes)
{
    ...
    while (1)
    {
        ...
        if (!prevbranch)
        {
            ...
            pxAssert(prevbranch || pbh->pblock == NULL);
...
static void SuperVUInitLiveness(VuBaseBlock *pblock)
{
    ...
    for (itinst = pblock->insts.begin(); itinst != pblock->insts.end(); ++itinst)
    {
        ...
        else
        {
            ...
            {
                if (itinst->regs[0].VFwxyzw != 0xf)
                    itinst->addvars[1] |= 1 << itinst->regs[0].VFwrite;
                else
                    vfwrite |= 1 << itinst->regs[0].VFwrite;
            }
            if (itinst->regs[1].VFwrite != 0)
            {
                if (itinst->regs[1].VFwxyzw != 0xf)
                    itinst->addvars[1] |= 1 << itinst->regs[1].VFwrite;
                else
                    vfwrite |= 1 << itinst->regs[1].VFwrite;
            }
    ...
    for (itinst = insts.begin(); itinst != insts.end(); itinst++)
    {
        ...
        for (int i = 1; i >= 0; --i)
        {
            ...
            if (reusereg >= 0)
            {
                ...
                xmmtype type = reusereg ? XMMTYPE_ACC : XMMTYPE_VFREG;
            ...
            if (itinst->vfwrite[i] >= 0)
                lastwrite = itinst->vfwrite[i];
            else if (itinst->vfacc[i] >= 0)
                lastwrite = itinst->vfacc[i];

            // always alloc at least 1 temp reg
            int free0 = (i || regs->VFwrite || regs->VFread0 || regs->VFread1 || (regs->VIwrite & (1 << REG_ACC_FLAG)) || (regs->VIread & (1 << REG_VF0_FLAG))) ? XMM_Reg.allocTemp(XMMT_FPS, -1) : -1;
            int free1 = 0, free2 = 0;
...
static u32 *SuperVUStaticAlloc(u32 size)
{
    ...
    // always zero
    if (size == 4)
        *(u32 *)recVUStackPtr[s_vu] = 0;
    else
        memset(recVUStackPtr[s_vu], 0, size);
...
void VuBaseBlock::Recompile()
{
...
#ifdef SUPERVU_X86CACHING
    if (nStartx86 >= 0)
        memcpy(X86_Reg.x86regs, &s_vecRegArray[nStartx86], sizeof(X86_Reg.x86regs));
    else
        X86_Reg.init();
#else
    X86_Reg.init();
    ...
    for (itinst = insts.begin(); itinst != insts.end(); itinst++)
    {
        ...
        if (s_JumpX86 > 0)
        {
            if (!X86_Reg.x86regs[s_JumpX86].inuse)
            {
                // load
                s_JumpX86 = X86_Reg.allocReg(xEmptyReg, X86TYPE_VUJUMP, 0, MODE_READ);
            }
            X86_Reg.x86regs[s_JumpX86].needed = 1;
        }

        if (s_ScheduleXGKICK && s_XGKICKReg > 0)
        {
            pxAssert(X86_Reg.x86regs[s_XGKICKReg].inuse);
            X86_Reg.x86regs[s_XGKICKReg].needed = 1;
        }
    ...
    else
    {
        ...
        // get rid of any writes, otherwise freeRegs will write
        X86_Reg.x86regs[s_JumpX86].mode &= ~MODE_WRITE;

        if (g_branch == 1)
        {
            if (!X86_Reg.x86regs[s_JumpX86].inuse)
            {
                pxAssert(X86_Reg.x86regs[s_JumpX86].type == X86TYPE_VUJUMP);
        ...
        if (nEndx86 >= 0)
        {
            ...
            for (int i = 0; i < iREGCNT_GPR; ++i)
            {
                ...
                if (endx86[i].inuse)
                {
                    ...
                    if (s_JumpX86 == i && X86_Reg.x86regs[s_JumpX86].inuse)
                    {
                        X86_Reg.x86regs[s_JumpX86].inuse = 0;
                        X86_Reg.x86regs[eax.GetId()].inuse = 1;
                        xMOV(eax, xRegister32(s_JumpX86));
                        s_JumpX86 = eax.GetId();
                    }

                    if (X86_Reg.x86regs[i].inuse)
                    {
                        if (X86_Reg.x86regs[i].type == endx86[i].type && X86_Reg.x86regs[i].reg == endx86[i].reg)
                        {
                            X86_Reg.freeReg(i);
...
#ifdef SUPERVU_INTERCACHING
                        if (X86_Reg.x86regs[i].type == (X86TYPE_VI | (s_vu ? X86TYPE_VU1 : 0)))
                        {
                            if (livevars[0] & (1 << X86_Reg.x86regs[i].reg))
                                X86_Reg.freeReg(i);
                            else
                                X86_Reg.x86regs[i].inuse = 0;
                        }
                        else
#endif
                        {
                    ...
                    // realloc
                    X86_Reg.allocReg(i, endx86[i].type, endx86[i].reg, MODE_READ);
                    if (X86_Reg.x86regs[i].mode & MODE_WRITE)
                    {
                        X86_Reg.freeReg(i);
                        X86_Reg.x86regs[i].inuse = 1;
                    }
                }
                else
                    X86_Reg.freeReg(i);
...
void VuInstruction::Recompile(std::list<VuInstruction>::iterator &itinst, u32 vuxyz)
{
    ...
    {
        if (X86_Reg.x86regs[j].inuse && X86_ISVI(X86_Reg.x86regs[j].type))
        {
            int count = 0;
            itinst2 = itinst;
            ...
            X86_Reg.x86regs[j].counter = 1000 - count;
        }
    }
#endif
    ...
    if (code_ptr[1] & 0x80000000) // I flag
    {
        ...
        if (s_JumpX86 > 0) X86_Reg.x86regs[s_JumpX86].needed = 1;
        if (s_ScheduleXGKICK && s_XGKICKReg > 0) X86_Reg.x86regs[s_XGKICKReg].needed = 1;
    ...
    else
    {
        if (regs[0].VIwrite & (1 << REG_Q))
        {
            ...
            int x86temp = -1;
            if (cacheq)
                x86temp = X86_Reg.allocReg(xEmptyReg, X86TYPE_TEMP, 0, 0);
        ...
        {
            int x86temp = X86_Reg.allocReg(xEmptyReg, X86TYPE_TEMP, 0, 0);
        ...
        {
            pxAssert(vicached >= 0);
            int cachedreg = X86_Reg.allocReg(xEmptyReg, (x86type)(X86TYPE_VI | (s_vu ? X86TYPE_VU1 : 0)), vicached, MODE_READ);
        ...
        //				}
        //
        //				oldreg = X86_Reg.allocReg(xEmptyReg, (x86type)(X86TYPE_VI|(s_vu?X86TYPE_VU1:0)), s_CacheVIReg, MODE_READ);
        //				s_CacheVIX86 = X86_Reg.allocReg(xEmptyReg, X86TYPE_VITEMP, s_CacheVIReg, MODE_WRITE);
        //				xMOV(xRegister32(s_CacheVIX86), xRegister32(oldreg));
        //			}
        //		}
        //		else if( pc == s_pCurBlock->endpc-8 && s_CacheVIReg >= 0 ) {
        //			pxAssert( s_CacheVIX86 > 0 && X86_Reg.x86regs[s_CacheVIX86].inuse && X86_Reg.x86regs[s_CacheVIX86].reg == s_CacheVIReg && X86_Reg.x86regs[s_CacheVIX86].type == X86TYPE_VITEMP );
        //
        //			oldreg = X86_Reg.allocReg(xEmptyReg, (x86type)(X86TYPE_VI|(s_vu?X86TYPE_VU1:0)), s_CacheVIReg, MODE_READ);
        //			X86_Reg.x86regs[s_CacheVIX86].needed = 1;
        //			pxAssert( X86_Reg.x86regs[oldreg].mode & MODE_WRITE );
        //
        //			X86_Reg.x86regs[s_CacheVIX86].type = X86TYPE_VI|(s_vu?X86TYPE_VU1:0);
        //			X86_Reg.x86regs[oldreg].type = X86TYPE_VITEMP;
        //		}
        ...
        //			// revert
        //			X86_Reg.x86regs[s_CacheVIX86].inuse = 0;
        //			X86_Reg.x86regs[oldreg].type = X86TYPE_VI|(s_vu?X86TYPE_VU1:0);
        //		}
...
void recVUMI_IBQ_prep()
{
    ...
    if (_Is_ == 0)
    {
...
#endif
        {
            itreg = X86_Reg.checkReg((x86type)(X86TYPE_VI | (VU == &VU1 ? X86TYPE_VU1 : 0)), _It_, MODE_READ);
        }

        s_JumpX86 = X86_Reg.allocReg(xEmptyReg, X86TYPE_VUJUMP, 0, MODE_WRITE);
        ...
        if (itreg >= 0)
        {
            ...
            xCMP(ptr16[(u16 *)(SuperVUGetVIAddr(_It_, 1))], 0);
    }
    else if (_It_ == 0)
    {
#ifdef SUPERVU_VIBRANCHDELAY
...
#endif
        {
            isreg = X86_Reg.checkReg((x86type)(X86TYPE_VI | (VU == &VU1 ? X86TYPE_VU1 : 0)), _Is_, MODE_READ);
        }
        ...
        if (isreg >= 0)
        {
            ...
            xCMP(ptr16[(u16 *)(SuperVUGetVIAddr(_Is_, 1))], 0);
    }
    else
    {
        X86_Reg.addNeededReg((x86type)(X86TYPE_VI | (VU == &VU1 ? X86TYPE_VU1 : 0)), _It_);
        ...
        if (s_pCurInst->vicached >= 0 && s_pCurInst->vicached == (s8)_It_)
        {
            ...
            if (isreg <= 0)
            {
                ...
                if (s_pCurInst->vicached >= 0 && s_pCurInst->vicached == (s8)_Is_)
                {
                    isreg = X86_Reg.allocReg(xEmptyReg, X86TYPE_TEMP, 0, MODE_READ | MODE_WRITE);
                    xMOV(xRegister32(isreg), ptr[(void *)(SuperVUGetVIAddr(_Is_, 1))]);
                }
                else
                    isreg = X86_Reg.allocReg(xEmptyReg, (x86type)(X86TYPE_VI | (VU == &VU1 ? X86TYPE_VU1 : 0)), _Is_, MODE_READ);
            }
        }
        else
#endif
...
void recVUMI_BAL(VURegs *vuu, s32 info)
{
    ...
    if (_It_)
    {
        X86_Reg.deleteReg((x86type)(X86TYPE_VI | (s_vu ? X86TYPE_VU1 : 0)), _It_, 2);
...
{
    int isreg = X86_Reg.allocReg(xEmptyReg, (x86type)(X86TYPE_VI | (s_vu ? X86TYPE_VU1 : 0)), _Is_, MODE_READ);
...
{
    X86_Reg.addNeededReg((x86type)(X86TYPE_VI | (s_vu ? X86TYPE_VU1 : 0)), _It_);
...
void recVUMI_XGKICK_(VURegs *VU)
{
    pxAssert(s_XGKICKReg > 0 && X86_Reg.x86regs[s_XGKICKReg].inuse && X86_Reg.x86regs[s_XGKICKReg].type == X86TYPE_VITEMP);

    X86_Reg.x86regs[s_XGKICKReg].inuse = 0; // so free doesn't flush
    X86_Reg.freeRegs();
    XMM_Reg.freeRegs();
...
void recVUMI_XGKICK(VURegs *VU, int info)
{
    ...
    int isreg = X86_Reg.allocReg(ecx, (x86type)(X86TYPE_VI | (s_vu ? X86TYPE_VU1 : 0)), _Is_, MODE_READ);
    X86_Reg.freeReg(isreg); // flush
    X86_Reg.x86regs[isreg].inuse = 1;
    X86_Reg.x86regs[isreg].type = X86TYPE_VITEMP;
    X86_Reg.x86regs[isreg].needed = 1;
    X86_Reg.x86regs[isreg].mode = MODE_WRITE | MODE_READ;
