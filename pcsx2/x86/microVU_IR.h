...
public:
    ...
    void TDwritebackAll(bool clearState = false)
    {
        for (int i = 0; i < xmmTotal; i++)
        {
            ...
            { // Reg was modified and not Temp or vf0
                if (mapX.VFreg == 33)
    ...
    void writeBackReg(const xmm &reg, bool invalidateRegs = true)
    {
        ...
        { // Reg was modified and not Temp or vf0
            ...
            if (mapX.xyzw == 0xf)
            { // Make Cached Reg if All Vectors were Modified
                mapX.count = counter;
                mapX.xyzw = 0;
                mapX.isNeeded = false;
                return;
        ...
        { // Clear reg if modified and is VF0 or temp reg...
            clearReg(reg);
        }
    }
    ...
    void clearNeeded(const xmm &reg)
    {
        ...
        { // Reg was modified
            if (clear.VFreg > 0)
            {
                ...
                for (int i = 0; i < xmmTotal; i++)
                { // Invalidate any other read-only regs of same vfReg
    ...
    const xmm &allocReg(int vfLoadReg = -1, int vfWriteReg = -1, int xyzw = 0, bool cloneWrite = 1)
    {
        ...
        { // Search For Cached Regs
            for (int i = 0; i < xmmTotal; i++)
            {
                ...
                { // Reg Had All Vectors Modified and != VF0
                    int z = i;

                    if (vfWriteReg >= 0)
                    { // Reg will be modified
                        if (cloneWrite)
                        { // Clone Reg so as not to use the same Cached Reg
                            z = findFreeReg();
                        ...
                        { // Don't clone reg, but shuffle to adjust for SS ops
        ...
        if (vfWriteReg >= 0)
        { // Reg Will Be Modified (allow partial reg loading)
        ...
        { // Reg Will Not Be Modified (always load full reg for caching)
            if (vfLoadReg == 33)
                loadIreg(xmmX, 0xf);
            else if (vfLoadReg == 32)
