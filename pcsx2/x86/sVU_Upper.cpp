...
void recUpdateFlags(VURegs *VU, int reg, int info)
{
    ...
    if (!(info & PROCESS_VU_UPDATEFLAGS))
    {
        ...
        {
            if (reg != EEREC_TEMP)
                vuFloat2(reg, EEREC_TEMP, _X_Y_Z_W);
            else
                vuFloat_useEAX(info, reg, _X_Y_Z_W);
        }
        return;
    }
    ...
    { // Checks all flags
        ...
        if (CHECK_VU_UNDERFLOW)
        {                                                              // Sets underflow/denormals to zero
    ...
    { // Only Checks for Sign and Zero Flags
    ...
    if (t1regBoolean == 2)
        xSHUF.PS(xRegisterSSE(reg), xRegisterSSE(reg), 0x1B); // Flip back reg to wzyx (have to do this because reg != EEREC_TEMP)
    else if (t1regBoolean == 1)
        xMOVAPS(xRegisterSSE(t1reg), ptr[TEMPXMMData]); // Restore data from temo address
    else
...
#undef PERFORM
    ...
    if (is_sub)
        xSUB.PS(xRegisterSSE(regd), xRegisterSSE(regt));
    else
        xADD.PS(xRegisterSSE(regd), xRegisterSSE(regt));
...
void VU_ADD_SUB_SS(u32 regd, u32 regt, int is_sub, int is_mem, int info)
{
    ...
    if (is_mem)
    {
        ...
        if (is_sub)
            xSUB.SS(xRegisterSSE(regd), ptr[&VU_addsub_reg[1][0]]);
        else
            xADD.SS(xRegisterSSE(regd), ptr[&VU_addsub_reg[1][0]]);
    }
    else
    {
        xAND.PS(xRegisterSSE(regd), ptr[&VU_addsub_reg[0][0]]); //regd contains mask
...
void SSE_ADDSS_M32_to_XMM_custom(int info, int regd, int regt)
{
    if (CHECK_VUADDSUBHACK)
    {
        ...
        xADD.SS(xRegisterSSE(regd), ptr[(void *)(regt)]);
}
void SSE_SUBSS_M32_to_XMM_custom(int info, int regd, int regt)
{
    if (CHECK_VUADDSUBHACK)
    {
        ...
        xSUB.SS(xRegisterSSE(regd), ptr[(void *)(regt)]);
}
//------------------------------------------------------------------
...
void recVUMI_ABS(VURegs *VU, int info)
{
    ...
    { // Use a temp reg because VU_MERGE_REGS() modifies source reg!
...
void recVUMI_ADD(VURegs *VU, int info)
{
    ...
    if (_Fs_ == 0 && _Ft_ == 0)
    { // if adding VF00 with VF00, then the result is always 0,0,0,2
        if (_X_Y_Z_W != 0xf)
        {
            ...
            xMOVAPS(xRegisterSSE(EEREC_D), ptr[s_two]);
    }
    else
    {
        if (CHECK_VU_EXTRA_OVERFLOW)
        ...
        if (_X_Y_Z_W == 8)
        { // If only adding x, then we can do a Scalar Add
            if (EEREC_D == EEREC_S)
                xADD.SS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_T));
            else if (EEREC_D == EEREC_T)
                xADD.SS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_S));
            else
            {
        ...
        { // If xyzw != 1111, then we have to use a temp reg
            xMOVAPS(xRegisterSSE(EEREC_TEMP), xRegisterSSE(EEREC_S));
            xADD.PS(xRegisterSSE(EEREC_TEMP), xRegisterSSE(EEREC_T));
            VU_MERGE_REGS(EEREC_D, EEREC_TEMP);
        }
        else
        { // All xyzw being modified (xyzw == 1111)
            if (EEREC_D == EEREC_S)
    ...
    else
    {
        ...
        else
        {
            if (EEREC_D == EEREC_TEMP)
                ...
                xADD.PS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_TEMP));
            else
            {
...
void recVUMI_ADDA(VURegs *VU, int info)
{
    ...
    if (_X_Y_Z_W == 8)
    {
        if (EEREC_ACC == EEREC_S)
            xADD.SS(xRegisterSSE(EEREC_ACC), xRegisterSSE(EEREC_T)); // Can this case happen? (cottonvibes)
        else if (EEREC_ACC == EEREC_T)
            xADD.SS(xRegisterSSE(EEREC_ACC), xRegisterSSE(EEREC_S)); // Can this case happen?
        else
        {
    ...
    {
        if (EEREC_ACC == EEREC_S)
            xADD.PS(xRegisterSSE(EEREC_ACC), xRegisterSSE(EEREC_T)); // Can this case happen?
        else if (EEREC_ACC == EEREC_T)
            xADD.PS(xRegisterSSE(EEREC_ACC), xRegisterSSE(EEREC_S)); // Can this case happen?
        else
        {
...
void recVUMI_ADDA_iq(VURegs *VU, uptr addr, int info)
{
    ...
    else
    {
        ...
        {
            if (EEREC_ACC == EEREC_S)
                xADD.PS(xRegisterSSE(EEREC_ACC), xRegisterSSE(EEREC_TEMP));
            else
            {
...
void recVUMI_SUB(VURegs *VU, int info)
{
    ...
    {
        if (_X_Y_Z_W != 0xf)
            xAND.PS(xRegisterSSE(EEREC_D), ptr[(&SSEmovMask[15 - _X_Y_Z_W][0])]);
        else
            xXOR.PS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_D));
    }
    else if (_X_Y_Z_W == 8)
    {
        if (CHECK_VU_EXTRA_OVERFLOW)
...
void recVUMI_SUBA(VURegs *VU, int info)
{
    ...
    {
        if (_X_Y_Z_W != 0xf)
            xAND.PS(xRegisterSSE(EEREC_ACC), ptr[(&SSEmovMask[15 - _X_Y_Z_W][0])]);
        else
            xXOR.PS(xRegisterSSE(EEREC_ACC), xRegisterSSE(EEREC_ACC));
    }
    else if (_X_Y_Z_W == 8)
    {
        if (EEREC_ACC == EEREC_S)
...
void recVUMI_MUL_xyzw_toD(VURegs *VU, int xyzw, int regd, int info)
{
    ...
    if (_Fs_)
    { // This is needed for alot of games; so always clamp this operand
        if (CHECK_VU_SIGN_OVERFLOW)
            vFloats4_useEAX[_X_Y_Z_W](EEREC_S, EEREC_TEMP); // Always clamp EEREC_S, regardless if CHECK_VU_OVERFLOW is set
        else
            vFloats2[_X_Y_Z_W](EEREC_S, EEREC_TEMP); // Always clamp EEREC_S, regardless if CHECK_VU_OVERFLOW is set
    }
    if (_Ft_ == 0)
    {
        if (xyzw < 3)
        {
            if (_X_Y_Z_W != 0xf)
            {
                ...
                xXOR.PS(xRegisterSSE(regd), xRegisterSSE(regd));
        }
        else
        {
            assert(xyzw == 3);
...
void recVUMI_MADD_iq_toD(VURegs *VU, uptr addr, int regd, int info)
{
    ...
    else
    {
        ...
        {
            if (regd == EEREC_ACC)
            { // ACCxyz is unchanged, ACCw <= ACCw + *addr
                if (_W)
                {                                                                       // if _W is zero, do nothing
            ...
            { // DESTxyz <= ACCxyz, DESTw <= ACCw + *addr
                if (_W)
                {
                    xMOVSSZX(xRegisterSSE(EEREC_TEMP), ptr[(void *)(addr)]);            // { *addr, 0, 0, 0 }
                    xSHUF.PS(xRegisterSSE(EEREC_TEMP), xRegisterSSE(EEREC_TEMP), 0x27); // { 0, 0, 0, *addr }
    ...
    if (_X_Y_Z_W != 0xf)
    {
        ...
        if (t1reg >= 0)
        {
            ...
            if (regd != EEREC_TEMP) { VU_MERGE_REGS(regd, t1reg); }
                ...
                xMOVAPS(xRegisterSSE(regd), xRegisterSSE(t1reg));
...
//mode1 = iq, mode2 = xyzw, mode0 = normal
{
    ...
    {
        t1regbool = 1;
        for (t1reg = 0; ((t1reg == EEREC_D) || (t1reg == EEREC_S) || (mode != 1 && t1reg == EEREC_T) || (t1reg == EEREC_TEMP)); t1reg++)
    ...
    else
    {
        ...
        if (_X_Y_Z_W == 8)
        {
            if (EEREC_D == EEREC_S)
                ...
                xMAX.SS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_S));
            else
            {
                xMOVSS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_S));
                xMAX.SS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_T));
...
void recVUMI_MAX_xyzw(VURegs *VU, int xyzw, int info)
{
    ...
    if (_Fs_ == 0 && _Ft_ == 0)
    {
        ...
        else if (_X_Y_Z_W != 0xf)
        {
            ...
            {
                if (_X_Y_Z_W & 1)
                    xMOVAPS(xRegisterSSE(EEREC_TEMP), ptr[(&VU->VF[0].UL[0])]); // w included, so insert the whole reg
                else
                    xXOR.PS(xRegisterSSE(EEREC_TEMP), xRegisterSSE(EEREC_TEMP)); // w not included, can zero out
            }
            else
                xMOVAPS(xRegisterSSE(EEREC_TEMP), ptr[s_fones]);
            VU_MERGE_REGS(EEREC_D, EEREC_TEMP);
        }
        else
        {
            ...
            if (xyzw < 3) { xMOVAPS(xRegisterSSE(EEREC_D), ptr[(&VU->VF[0].UL[0])]); }
                ...
                xMOVAPS(xRegisterSSE(EEREC_D), ptr[s_fones]);
        }
        return;
    }
    ...
    else
    {
        ...
        if (_X_Y_Z_W == 8)
        {
            if (EEREC_D == EEREC_S)
                ...
                xMIN.SS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_S));
            else
            {
                xMOVSS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_S));
                xMIN.SS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_T));
...
void recVUMI_MINI_xyzw(VURegs *VU, int xyzw, int info)
{
    ...
    if (_Fs_ == 0 && _Ft_ == 0)
    {
        if (_X_Y_Z_W == 0xf)
        {
            ...
            if (xyzw == 3)
                xMOVAPS(xRegisterSSE(EEREC_D), ptr[(&VU->VF[0].UL[0])]);
            else
                xXOR.PS(xRegisterSSE(EEREC_D), xRegisterSSE(EEREC_D));
        }
        else
        {
            //If VF0.w is the constant, the result will match VF0, else its all 0's
            if (xyzw == 3)
                xMOVAPS(xRegisterSSE(EEREC_TEMP), ptr[(&VU->VF[0].UL[0])]);
            else
                xXOR.PS(xRegisterSSE(EEREC_TEMP), xRegisterSSE(EEREC_TEMP));
            VU_MERGE_REGS(EEREC_D, EEREC_TEMP);
        }
...
void recVUMI_FTOI0(VURegs *VU, int info)
{
    ...
    if (_X_Y_Z_W != 0xf)
    {
        ...
        if (t1reg >= 0)
        { // If theres a temp XMM reg available
