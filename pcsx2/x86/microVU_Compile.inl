...
void mVUsetupRange(microVU &mVU, s32 pc, bool isStartPC)
{
    ...
    if (isStartPC)
    { // Check if startPC is already within a block we've recompiled
...
__fi void handleBadOp(mV, int count)
{
    ...
    if (mVUinfo.isBadOp)
    {
        ...
        if (!isVU1)
            xFastCall(mVUbadOp0, mVU.prog.cur->idx, xPC);
        else
            xFastCall(mVUbadOp1, mVU.prog.cur->idx, xPC);
...
void mVUoptimizePipeState(mV)
{
    ...
    if (mVUregs.q)
    {
        optimizeReg(mVUregs.q);
        if (!mVUregs.q) { incQ(mVU); }
    }

    if (mVUregs.p)
    {
        optimizeReg(mVUregs.p);
        if (!mVUregs.p) { incP(mVU); }
    }
...
void mVUincCycles(mV, int x)
{
    ...
    if (mVUregs.q)
    {
        if (mVUregs.q > 4)
        {
            ...
            if (mVUregs.q <= 4) { mVUinfo.doDivFlag = 1; }
        }
        else
        {
            calcCycles(mVUregs.q, x);
...
void cmpVFregs(microVFreg &VFreg1, microVFreg &VFreg2, bool &xVar)
{
    ...
    {
        if ((VFreg1.x && VFreg2.x) || (VFreg1.y && VFreg2.y) || (VFreg1.z && VFreg2.z) || (VFreg1.w && VFreg2.w))
        { xVar = 1; }
    }
}
...
void *mVUcompile(microVU &mVU, u32 startPC, uptr pState)
{
    ...
    for (; x < endCount; x++)
    {
        ...
        else
        {
            ...
            {
                case 1:
                case 2: normBranch(mVU, mFC); goto perf_and_return; // B/BAL
                case 9:
                case 10: normJump(mVU, mFC); goto perf_and_return;                      // JR/JALR
...
_mVUt void *__fastcall mVUcompileJIT(u32 startPC, uptr ptr)
{
    ...
    { // Treat jump as part of same microProgram
