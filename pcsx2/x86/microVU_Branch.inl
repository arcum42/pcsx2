...
{
    if (!microVU0.prog.cleared) memzero(microVU0.prog.lpState);
}

void mVU1clearlpStateJIT()
{
    if (!microVU1.prog.cleared) memzero(microVU1.prog.lpState);
}
...
void normJumpCompile(mV, microFlagCycles &mFC, bool isEvilJump)
{
    ...
    if (isEvilJump)
        xMOV(gprT2, ptr32[&mVU.evilBranch]);
    else
        xMOV(gprT2, ptr32[&mVU.branch]);

    if (doJumpCaching)
        xMOV(gprT3, (uptr)mVUpBlock);
    else
    ...
    if (!mVU.index)
        xFastCall((void *)(void (*)())mVUcompileJIT<0>, gprT2, gprT3); //(u32 startPC, uptr pState)
    else
        xFastCall((void *)(void (*)())mVUcompileJIT<1>, gprT2, gprT3);
