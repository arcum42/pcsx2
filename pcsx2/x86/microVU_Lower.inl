...
mVUop(mVU_DIV)
{
    ...
    pass2
    {
        ...
        if (_Ftf_)
            Ft = mVU.regAlloc->allocReg(_Ft_, 0, (1 << (3 - _Ftf_)));
        else
...
mVUop(mVU_LQD)
{
    ...
    pass2
    {
        ...
        if (_Is_ || isVU0)
        { // Access VU1 regs mem-map in !_Is_ case
            mVUallocVIa(mVU, gprT2, _Is_);
            xSUB(gprT2b, 1);
...
mVUop(mVU_SQD)
{
    ...
    pass2
    {
        ...
        if (_It_ || isVU0)
        { // Access VU1 regs mem-map in !_It_ case
            mVUallocVIa(mVU, gprT2, _It_);
            xSUB(gprT2b, 1);
...
void setBranchA(mP, int x, int _x_)
{
    ...
    pass2
    {
        if (_Imm11_ == 1 && !_x_ && !isBranchDelaySlot) { return; }
        mVUbranch = x;
    }
    pass3 { mVUbranch = x; }
    pass4
